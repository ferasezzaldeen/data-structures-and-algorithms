...
// Solution code here...
  let sol = arr.map(result => result.length).indexOf(Math.max(...arr.map(result => result.length)));
  return sol;
};
...
const findHappiness = (arr) => {
  // Solution code here...
  let sol = arr.filter(result => result.includes(':)'));
...
const standardizePhoneNumbers = (arr) => {
  // Solution code here...
  let sol = arr.map(result => result.substring(1, 4) + result.substring(6, 9) + result.substring(10, 14));
  return sol;
};
...
const onlyOddChars = (str) => {
  // Solution code here...
  let sol = str.split('').reduce((acc, val, idx) => (idx % 2 !== 0) ? acc += val : acc, '');
  return sol;
};
...
const allHappy = (arr) => {
  // Solution code here...
  let sol = ((arr.filter(result => !(result.includes(':)'))).length > 0) ? false : true);
  return sol;
};
