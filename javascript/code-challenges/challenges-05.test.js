...
const toLastNames = people => {
  ...
  let sol=[];
  people.map((ind)=>{
    let name=ind.firstName+' '+ind.lastName;
    sol.push(name);
...
const addPurchases = (arr) => {
  // Solution code here...
  let sol = arr.reduce( (accumulator, person, idx) => {
    accumulator = accumulator+person.purchasePrice;
    return accumulator;
  }, 0 );return sol;
};
...
const countNumberOfElements = (arr) => {
  // Solution code here...
  let sol = arr.reduce( (accumulator) => {
...
const returnNames = (arr) => {
  // Solution code here...
  let sol = arr.reduce( (accumulator, person) => {
...
const reversedString = (str) => {
  ...
  let arr=str.split('');
  let sol = arr.reduce( (accumulator, person) => {
    accumulator.unshift(person);
    return accumulator;
  }, [] );
  let nstr=sol.toString();
  while(nstr.indexOf(',') > -1)
  {
    nstr=nstr.replace(',', '');
  }
  // let newStr = nstr.replace(',', '');
  return nstr;
